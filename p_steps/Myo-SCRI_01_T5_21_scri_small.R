...
for (subpop in subpopulations_non_empty) {
  ...
  sdr0 <- paste0(direxpsubpop[[subpop]], "scri_small/")
  dir.create(file.path(sdr0), showWarnings = FALSE, recursive = TRUE)
  
  sdr0_flowchart <- paste0(direxpsubpop[[subpop]], "scri_small/flowchart_for_vax/")
  dir.create(file.path(sdr0_flowchart), showWarnings = FALSE, recursive = TRUE)
  
  
  # scri output_directory for models NOT for export:
  sdr_models0 <- paste0(diroutput, "scri_small/")
  if(length(subpopulations_non_empty)>1) sdr_models0 <- paste0(diroutput, "scri_small/",subpop,"/")
  ...
  extra_options <- list( lparal               = lparal,    # if T ==> library(parallel) is started in function 'scri'
                         ...
                         leventplot           = F,   # leventplot, ==> no image and cohort plots
  ...
  for(iae in ae_events){
    ...
    if(lmain){
      ...
      vax_def0        <- scri_data_parameters( data =  data_vax, vax_name  = "vax_number",  vax1="dose 1",  vax_time = "vax_days",        vax_date      = "vax_date", 
                                               #vax_name_v1  = "vax_number_v1",    vax_time_v1 = "vax_days_v1",  vax_date_v1   = "vax_date_v1", 
    ...
    #########################################################################
    #
    #  this part:  also with time adjustment
    #
    if(ldist){
