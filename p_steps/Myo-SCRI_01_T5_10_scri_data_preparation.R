# Program Information  ----------------------------------------------------
#
# Program:      Myo-SCRI_01_T5_10_scri_data_preparation.R 
...
#               input:   g_intermediate/D3_study_population_SCRI.RData  
#               output:  g_intermediate/data_vax_SCRI 
#                        g_output/excluded_rows.RData
#                        g_output/excluded_rows_2.RData
#                        g_output/excluded_rows_wrong_death_date.RData
...
if(!any(ls()=="id_original")) id_original <- id
...
for (subpop in subpopulations_non_empty) {
 ...
 #  max_number_doses <- 4
  ...
  # delete vaccinated persons minder dan 28 days before the 'recommended_end_date':
  dim(data_vax)
  data_vax <- data_vax[ as.numeric( difftime( as.Date(recommended_end_date), data_vax$vax_date, units="days")) >= 28, ]
  dim(data_vax)
  
  
  if(all("death_date"!=names(data_vax))){
    if("date_of_death" %in% names(data_vax))    data_vax$death_date <- data_vax$date_of_death
    else if("date_death" %in% names(data_vax))  data_vax$death_date <- data_vax$date_death
  }
  ...
  if(length(tb)>1){  
    warning(paste("There are ",sum(!cond,na.rm=T),"rows (",sum(!cond[data_vax$vax_n<=1],na.rm=T),"id's) with 'study_entry_date' >= 'study_exit_date' !"))
    ...
    save(data_vax_excluded_wrong_study_period,file=paste0(sdr_models0,"excluded_rows_wrong_study_period.RData"))
    sink(paste0(sdr_models0,"excluded_rows_wrong_study_period.txt")); old_sink = options (width=300, max.print=99999 );print(data_vax_excluded_wrong_study_period);options(old_sink);sink()
  ...
  if(any(!cond)){
    # table(as.numeric(difftime(data_vax_excluded_wrong_death_date$date_of_death, data_vax_excluded_wrong_death_date$study_exit_date,units="days"))[!duplicated(data_vax_excluded_wrong_death_date$person_id)])
    # hist(as.numeric(difftime(data_vax_excluded_wrong_death_date$date_of_death, data_vax_excluded_wrong_death_date$study_exit_date,units="days"))[!duplicated(data_vax_excluded_wrong_death_date$person_id)],10000)
    # hist(as.numeric(difftime(data_vax_excluded_wrong_death_date$date_of_death, data_vax_excluded_wrong_death_date$study_exit_date,units="days"))[!duplicated(data_vax_excluded_wrong_death_date$person_id)],2000,xlim=c(0,100)); grid()
    # 
    warning(paste( sum(!cond), "rows (",sum(!cond[data_vax$vax_n<=1],na.rm=T),"id's) with the date of death not in ['study_entry_days';'study_exit_days']"))
    data_vax_excluded_wrong_death_date <- data_vax[!cond,]
    save(data_vax_excluded_wrong_death_date,file=paste0(sdr_models0,"excluded_rows_wrong_death_date.RData"))
    sink(paste0(sdr_models0,"excluded_rows_wrong_death_date.txt")); old_sink = options (width=300, max.print=99999 );print(data_vax_excluded_wrong_death_date);options(old_sink);sink()
  ...
  if(any(!cond)){
    warning(paste( sum(!cond), "rows (",sum(!cond[data_vax$vax_n<=1],na.rm=T),"id's) with the vaccination date not in ['study_entry_days';'study_exit_days']"))
    data_vax_excluded_wrong_vax_date <- data_vax[!cond,]
    save(data_vax_excluded_wrong_vax_date,file=paste0(sdr_models0,"excluded_rows_wrong_vax_date.RData"))
    sink(paste0(sdr_models0,"excluded_rows_wrong_vax_date.txt")); old_sink = options (width=300, max.print=99999 );print(data_vax_excluded_wrong_vax_date);options(old_sink);sink()
  ...
  data_vax[is.na(data_vax$dist_gt_60),"dist_gt_60"] <- "no_d."
  
  #### 'next_vax_days'
  data_vax$next_vax_days <- data_vax[,"vax_days"] + c(diff(data_vax[,"vax_days"]), NA)
  data_vax[ c(data_vax[-nrow(data_vax),id]!=data_vax[-1,id], F), "next_vax_days"  ] <- NA
  ...
  save(data_vax, file=paste0(dirtemp, "scri/data_vax_SCRI", suffix[[subpop]], ".RData"))
