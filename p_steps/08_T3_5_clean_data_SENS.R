...
for (subpop in subpopulations_non_empty) {
  ...
  if(any(scri_data_extract$start_study_date > scri_data_extract$date_vax1 & !is.na(scri_data_extract$date_vax1) & !is.na(scri_data_extract$start_study_date), na.rm=T )){
    ...
    scri_data_extract$vacc_cond <- scri_data_extract$start_study_date <= scri_data_extract$date_vax1 & !is.na(scri_data_extract$date_vax1) | is.na(scri_data_extract$date_vax1) 
    scri_data_extract <- scri_data_extract[scri_data_extract$vacc_cond,]  # if study_entry_days > vax1 ==>  delete rows
  ...
  if(any(scri_data_extract$study_exit_days < scri_data_extract$days_vax1 & !is.na(scri_data_extract$days_vax1), na.rm = T)) {
    ...
    nrow0<-nrow(scri_data_extract)  
    scri_data_extract$exit_cond <- scri_data_extract$days_vax1 <= scri_data_extract$study_exit_days | is.na(scri_data_extract$days_vax1 )
    scri_data_extract <- scri_data_extract[scri_data_extract$exit_cond,]  # if study_exit_days < vax1 ==>  delete rows
  }  
